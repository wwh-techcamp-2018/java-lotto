...
public class LottoTest {
    ...
    public void matchNumberTest() {
        Lotto lotto = new Lotto(Sets.newHashSet(Arrays.asList(1, 2, 3, 4, 5, 6)));

        lotto.matchState(new Lotto(Sets.newHashSet(Arrays.asList(1, 2, 3, 7, 8, 9))));
    ...
    public void createInvalidLottoTest_숫자개수_불일치() {
        Lotto lotto = new Lotto(Sets.newHashSet(Arrays.asList(1, 2, 3, 4, 5)));
    }

    @Test(expected = IllegalArgumentException.class)
    public void createInvalidLottoTest_숫자범위_불일치() {
        Lotto lotto = new Lotto(Sets.newHashSet(Arrays.asList(1, 2, 3, 4, 5, 60)));
    }

    @Test(expected = IllegalArgumentException.class)
    public void createInvalidLottoTest_숫자중복_체크() {
        Lotto lotto = new Lotto(Sets.newHashSet(Arrays.asList(1, 2, 3, 4, 5, 5)));
    }
